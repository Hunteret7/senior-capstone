...
export class FarmsComponent implements OnInit {
    ...
    saveProduct() {
        console.log("hello")
        this.http.post('https://native-plants-backend.herokuapp.com/i/INSERT INTO rev2.farms(farm_name) VALUES (%s) / hunteret farmss', '{}')
            .subscribe(Response => {
                let res = JSON.parse(JSON.stringify(Response))
                if (res.success) {
                    alert("Successfully added the farm to the database")
                } else {
                    alert("Adding the farm failed. The console may have more details.")
                    console.log("Failed to push object to the database.\n==Response\n", res)
                }
            });
    }
}